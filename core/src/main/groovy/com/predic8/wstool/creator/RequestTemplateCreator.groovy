...
class RequestTemplateCreator extends AbstractSchemaCreator<RequestTemplateCreatorContext> {
    ...
    public Map<String, TemplateElementInfo> elementsInfo  = new HashMap<String, TemplateElementInfo>();
    ...
    void createElement(Element element, RequestTemplateCreatorContext ctx) {
        ...
        if (element.type) {
            ...
            if (refType && (refType instanceof BuiltInSchemaType)) {
                ...
                TemplateElementInfo info = getInfo(ctx.path.toString() + element.name)
                info.minOccurs = element.minOccurs
                info.maxOccurs = element.maxOccurs
                info.type = element.type.localPart
    ...
    private TemplateElementInfo getInfo(String xpath) {
        elementsInfo.computeIfAbsent(xpath, new Function<String, TemplateElementInfo>() {
            @Override
            TemplateElementInfo apply(String s) {
                return new TemplateElementInfo()
            }
        })
    }
    ...
    void createPart(part, RequestTemplateCreatorContext ctx) {
        ...
        if (part.type instanceof BuiltInSchemaType) {
            ...
            def info = getInfo(ctx.path.toString() + part.name)
            info.type = part.type.qname.localPart
