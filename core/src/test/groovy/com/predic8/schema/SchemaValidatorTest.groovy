...
class SchemaValidatorTest extends GroovyTestCase {
  ...
  void setUp(){
	...
	void testComplexTypeGetSuperTypes() {
		...
		assert ctx.errors[1].message == "Element street uses '{http://predic8.com/common/1/}Street4Test' as its reference, which is not defined in this schema."
		...
		assert ctx.errors[5].message == "ComplexType GrandParent inherits from '{http://predic8.com/wsdl/material/ArticleService/1/}GrandX3Parent', which is not definded in this schema."
		assert ctx.errors[6].message == "ComplexType Parent inherits from '{http://predic8.com/wsdl/material/ArticleService/1/}GrandX3Parent', which is not definded in this schema."
		assert ctx.errors[7].message == "ComplexType Child inherits from '{http://predic8.com/wsdl/material/ArticleService/1/}GrandX3Parent', which is not definded in this schema."
	}
}
